...
bool UFOLayer::init()
{
	...
	do 
	{
		...
		this->schedule(schedule_selector(UFOLayer::AddBigBoom),20.0,(1 <<30 ),5.0);
		bRet=true;
	} while (0);
...
void UFOLayer::AddMutiBullets(float dt)
{
	...
	CCSprite* mutiBullets=CCSprite::createWithSpriteFrame(CCSpriteFrameCache::getInstance()->spriteFrameByName("ufo1.png"));
...
{
	CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("sound/out_porp.mp3");
	CCSprite* bigBoom=CCSprite::createWithSpriteFrame(CCSpriteFrameCache::getInstance()->spriteFrameByName("ufo2.png"));
